...
# Analyze file information k0de stolen from: http://redneckprogrammer.blogspot.com/2007/09/word-character-line-counter-ruby-script.html
...
# TODO: Need to add regex support or ability to scan for multiple strings like 160 of them.
puts "\nStep 2/3:".foreground(:white).underline.bright << " Starting security analysis on #{filename}...".foreground(:cyan)
File.open(ARGV[0]) do |f|
  f.each_line do |line|
    if line.match(/(sprintf|strcpy|strcpyA|scanf)/i)
      puts "The following potentially dangerous methods were found:::".foreground(:red)
      puts "#{line}".foreground(:white)
    end
  end
end

# TODO: This must be tied into the above way I use to scan for multiple values vs 1 in order for the % scoring to work!
dfuncs = %w{sprintf}
...
puts "Score guidelines are: <= (less than) 97% horrible code, >= (greater than) 99% secure code.".foreground(:cyan)
